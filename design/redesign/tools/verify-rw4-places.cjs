...
async function inbox(page) {
  ...
  await page.waitForSelector('#main [data-act="xdo"]', { timeout: 8000 }).catch(() => {});
  ...
  await page.click(`#main [data-act="xdo"][data-id="${first.id}"][data-v="denied"]`);
  ...
  check("xdo Don’t: the request is declined (GET /api/flows-boards/installs)", declined?.status === "declined", declined?.status);
  await sleep(800);
  await page.click(`#main [data-act="xdo"][data-id="${second.id}"][data-v="allowed"]`);
  const allowed = await until("the yes", async () => (await get("flows-boards/installs")).requests.find((r) => r.id === second.id && r.status !== "waiting"), 8000).catch(() => null);
  check("xdo Allow: the request is approved, with its next step (GET /api/flows-boards/installs)", allowed?.status === "approved" && Boolean(allowed?.nextStep), allowed?.status);
  const toast = await page.locator(".toast").last().textContent({ timeout: 4000 }).catch(() => "");
  check("xdo Allow: the engine's next step is shown", toast.includes(allowed?.nextStep?.slice(0, 30) ?? "\u0000"));
