...
function card([id, name, group, mine], mode) {
  const acts = mine ? `<span class="my-acts"><button type="button" data-act="my-edit" data-v="${esc(mine.id)}">Edit</button><button type="button" data-act="my-dup" data-v="${esc(mine.id)}">Copy</button><button type="button" data-act="my-code" data-v="${esc(mine.id)}">Code</button><button type="button" data-act="my-del" data-v="${esc(mine.id)}">Delete</button></span>` : "";
  const on = wornId() === id;
  return `<div class="theme6" aria-current="${on}"><button class="theme6-b" type="button" data-act="skin" data-v="${esc(id)}" aria-pressed="${on}" aria-label="${esc(name)}">${swatch(lookEF(id, mode))}<b>${esc(name)}</b><small>${esc(mine ? "Yours" : group)}</small></button>${acts}</div>`;
}
...
function galleryBody() {
  ...
  const tabs = groups.map((g) => `<button class="tab" type="button" aria-selected="${g === G.f}" data-act="skinf" data-v="${g}">${g}${g === "Yours" ? `<span class="n">${L.my.length}</span>` : ""}</button>`).join("");
  const make = G.f === "Yours" || G.f === "All" ? `<button class="theme6 make" type="button" data-act="ce-new">${ic("palette")}<b>Make your own</b><small>Pick every colour</small></button>` : "";
  const shown = (L.cat?.THEMES ?? []).length;
  return `<div class="gal-top"><span class="tabs" data-css="margin:0">${tabs}</span></div>
    <div class="gal-bar"><label class="set-search" data-css="flex:1;margin:0">${ic("search", "s")}<input id="skin-q" placeholder="Search ${all.length} themes" value="${esc(G.q)}" aria-label="Search themes"></label>
      <span class="seg" role="group" aria-label="Show the themes in">${[["light", "Daylight"], ["dark", "Moonlight"]].map(([v, l]) => `<button type="button" data-act="skinprev" data-v="${v}" aria-pressed="${mode === v}">${l}</button>`).join("")}</span></div>
    <div class="themes6">${list.map((x) => card(x, mode)).join("") || '<p class="hint">No theme has that name.</p>'}${make}</div>
    <div class="gal-foot"><label class="chk"><input type="checkbox" id="g-contrast" data-sw="contrast"> More contrast</label><span class="grow"></span><button class="btn sm" type="button" data-act="my-paste">Paste a theme code</button><button class="btn pri sm" type="button" data-act="ce-new">${ic("palette", "s")}Make your own</button></div>
    ${shown ? `<p class="hint" data-css="margin:0">The ${shown} themes are the same colours keepoak.com shows, each in Daylight and Moonlight. Picking one keeps your light or dark choice.</p>` : ""}`;
}
...
export function skinGallery(filter) {
  ...
  if (!open) { openDlg({ title: "Themes", wide: true, body: galleryBody() }); paintContrast(); return; }
...
function startCed(fromId, editId) {
  ...
  const name = mine ? mine.name : base === BASE ? "My theme" : `My ${lookOf(base)[1]}`;
...
function ratings(c) {
  const r = (a, b, need) => { const x = contrastC(a, b); return [x, x >= need + 2.5 ? "Easy to read" : x >= need ? "Readable" : "Hard to read", x >= need]; };
  return [["Text on the background", ...r(c.ink, c.bg, 4.5)], ["Softer text on the background", ...r(c.ink2, c.bg, 4.5)], ["Text on cards", ...r(c.ink, c.raise, 4.5)], ["Button text on buttons", ...r(c.onBtn, c.btn, 4.5)], ["Accent against the background", ...r(c.accent, c.bg, 3)]];
}
...
function preview(c) {
  ...
  return `<div class="cprev" data-css="${css}"><div class="cp-side"><b>Branch</b>${rows}</div>
    <div class="cp-main">${talk}<span class="cp-chips"><i class="c-ok">Done</i><i class="c-warn">Estimate</i><i class="c-bad">Stopped</i></span></div></div>
...
function openCed() {
  ...
  const rows = EF.map(([k, l]) => `<label class="crow"><input type="color" id="ce-${k}" value="${c[k]}" aria-label="${esc(l)}"><span>${esc(l)}</span><input class="inp hexin" id="ceh-${k}" value="${c[k]}" maxlength="7" aria-label="${esc(l)} as a hex code" spellcheck="false"></label>`).join("");
  const bases = looks().filter((x) => !x[3]).map(([id, n]) => `<option value="${esc(id)}">${esc(n)}</option>`).join("");
  const dlg = openDlg({ title: d.id ? `Edit ${d.name}` : "Make your own theme", wide: true, body: `<div class="ced">
    <div class="ced-l"><label class="fld"><span>Name</span><input class="inp" id="ce-name" value="${esc(d.name)}" maxlength="40"></label>
      <label class="fld"><span>Start from</span><select class="inp" id="ce-base">${bases}</select></label>
      <div class="fld"><span>You are colouring</span><span class="seg" role="group" aria-label="Which mode you are colouring">${[["light", "Daylight"], ["dark", "Moonlight"]].map(([v, l]) => `<button type="button" data-act="ce-mode" data-v="${v}" aria-pressed="${d.edit === v}">${l}</button>`).join("")}</span></div>
      <div class="fld"><span>Accent</span><span class="accs">${ACCENTS.map((a) => `<button type="button" class="acc" data-css="--c:${a}" data-act="ce-acc" data-v="${a}" aria-label="Accent ${a}" aria-pressed="${c.accent.toUpperCase() === a}"></button>`).join("")}</span></div>
      <div class="crows">${rows}</div>
      <button class="btn sm" type="button" data-act="ce-fill">${ic("spark", "s")}Fill in the rest from background, text and accent</button></div>
    <div class="ced-r"><p class="hint" data-css="margin:0 0 8px">Changes show on the whole window as you pick. Each theme has a Daylight and a Moonlight side; colour both, or leave the other as it came.</p><div id="ce-prev">${preview(c)}</div></div></div>`,
  foot: `<button class="btn ghost" type="button" data-act="ce-cancel">Cancel</button><button class="btn pri" type="button" data-act="ce-save">${d.id ? "Save changes" : "Save theme"}</button>` });
...
function cedEnd(keep) {
  ...
  const name = ($("#ce-name")?.value || d.name).trim().slice(0, 40) || "My theme";
  ...
  applyLook();
  renderNow();
  toast(`${name} is saved and on. Find it under Themes › Yours.`);
}
...
function askDelete(id) {
  const x = L.my.find((y) => y.id === id);
  if (x) openDlg({ title: `Delete ${x.name}?`, body: '<p data-css="margin:0">It is taken off this computer. Anything using it goes back to Branch Slate.</p>', foot: `<button class="btn ghost" type="button" data-act="skins">Keep it</button><button class="btn dz" type="button" data-act="my-del-yes" data-v="${esc(x.id)}">Delete</button>` });
}
...
export function initThemes() {
  ...
  on("my-paste", () => openDlg({ title: "Paste a theme code", body: '<textarea class="inp code6" id="paste6" rows="6" placeholder=\'{"branchTheme":1,"name":…}\' aria-label="Theme code"></textarea><p class="hint" id="paste6-why" data-css="margin:6px 0 0"></p>', foot: '<button class="btn ghost" type="button" data-act="skins">Cancel</button><button class="btn pri" type="button" data-act="my-paste-go">Add theme</button>' }));
