...
export function draw() {
  ...
  <div class="fld" data-css="margin-top:6px"><span>Whose files</span><span class="acts" data-css="gap:6px"><button class="chip6" type="button" data-act="if-owner" data-v="branch" aria-pressed="${selectedOwner === "branch"}">Every Trunk</button>${E.trunks.map((t) => `<button class="chip6" type="button" data-act="if-owner" data-v="${esc(t.id)}" aria-pressed="${selectedOwner === t.id}">${esc(t.name)}</button>`).join("")}</span></div>
  ...
  if (files && files.files) {
    for (const f of files.files) {
      ...
      html += `<div class="prow"><code class="if-name">${esc(f.name)}</code><span class="grow"><b data-css="font-weight:500">${esc(f.description)}</b><small>${sizeText}</small></span><button class="btn sm" type="button" data-act="if-open" data-f="${esc(f.name)}">${statusText}</button></div>`;
    }
  }
