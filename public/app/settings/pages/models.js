...
const num = (label, unit) => `<span class="right num15"><input class="inp" aria-label="${label}">${unit ? `<small>${unit}</small>` : ""}</span>`;
...
const ADVANCED = `<div class="sec x15-sec"><h2>Budgets</h2>${row("Most steps in one task", num("Most steps in one task", "steps"), "It stops and asks when it gets there.")}${row("Spend cap per task", num("Spend cap per task", "USD"), "Only for accounts that bill per use.")}${row("Sub-tasks at once", seg("Sub-tasks at once", ["1", "3", "5"]), "Parts of a big task that can run side by side.")}</div>`
  + `<div class="sec x15-sec"><h2>Models for smaller jobs</h2>${row("Sub-tasks and side jobs", seg("Sub-tasks and side jobs", ["Same model"]), "Titles, summaries and searches inside a task.")}${sw("f15-pick-the-model-per-task", "Pick the model per task", "Easy tasks go to a quick model, hard ones to the best you have.")}${row("Planning model", seg("Planning model", ["Same model"]), "Writes the plan in Plan first.")}${sw("f15-mix-models-on-hard-questions", "Mix models on hard questions", "Asks two and merges the best of each. Off until you choose: it doubles the cost.")}</div>`
