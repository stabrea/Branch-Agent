...
export function draw() {
  ...
  <div class="sec ideas15"><div class="sec-h15"><h2>Ideas</h2></div></div>`;
    ...
    <div class="rows" data-css="margin-top:8px">${procedures.length ? procedures.map((p, i) => `<div class="prow" data-act="flow" data-id="${esc(p.id)}">${av({}, 34)}<span class="grow"><b>${esc(p.name || 'Procedure')}</b><small>${esc((p.steps || '0 steps') + ' · Branch')}</small></span><button class="btn sm" type="button" data-act="toast" data-msg="Running ${esc(p.name || 'Procedure')}…">Run now</button></div>`).join('') : ''}</div>
