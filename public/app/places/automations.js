...
export function draw() {
  ...
  <div class="sec ideas15"><div class="sec-h15"><h2>Ideas</h2><button type="button" class="link15" data-act="ideas15">See all ${IDEAS.length}</button></div><div class="idea-row15">${IDEAS.slice(0, 3).map(ideaCard).join('')}</div></div>`;
    ...
    <div class="rows" data-css="margin-top:8px">${procedures.length ? procedures.map((p, i) => `<div class="prow">${av({}, 34)}<span class="grow"><b>${esc(p.data?.definition?.name ?? '')}</b><small>${esc(p.data?.status ?? '')}</small></span><button class="btn sm" type="button" data-act="toast">Run now</button></div>`).join('') : ''}</div>
