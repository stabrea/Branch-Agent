...
const WORD = { model: "Model call", tool: "Tool", ask: "Approval", helper: "Helper", you: "You" };
...
function view(step, data) {
  ...
  const tokens = step.tokens && (step.tokens.input != null || step.tokens.output != null) ? `${nums(step.tokens.input)} words in, ${nums(step.tokens.output)} out` : "";
  const title = step.kind === "helper" ? `Started a helper: ${step.title}` : step.title;
  const detail = step.kind === "model" ? [step.detail, tokens].filter(Boolean).join(" · ") : step.detail;
  const who = step.kind === "you" ? "You" : helper ? helper.name || "" : author();
...
function head(data, steps) {
  ...
  const ticks = steps.map((s, i) => `<button type="button" class="tk17c k${KIND[s.kind]}17c ${i === T.at ? "on17c" : ""} ${i > T.at ? "later17c" : ""}" data-act="tlgo17c" data-v="${i}" aria-label="Step ${i + 1}: ${esc(view(s, data).title)}"></button>`).join("");
  return `<div class="tlh17c"><b>${esc(data.title)}</b><small>${steps.length} steps · ${dur(time)}${live ? " so far" : ""}${cost}</small></div>
    <div class="tlctl17c"><button type="button" class="icon-btn" data-act="tlstep17c" data-v="-1" aria-label="Step back" ${T.at <= 0 ? "disabled" : ""}>${ic("back17c", "s")}</button><button type="button" class="btn sm tlplay17c" data-act="tlplay17c" aria-pressed="${T.on}">${ic(T.on ? "pause15" : "play15", "s")}${T.on ? "Pause" : "Play"}</button><button type="button" class="icon-btn" data-act="tlstep17c" data-v="1" aria-label="Step forward" ${T.at >= steps.length - 1 ? "disabled" : ""}>${ic("fwd17c", "s")}</button><span class="tkrow17c" role="group" aria-label="Steps">${ticks}</span></div>`;
}

const PILL = { waiting: ["work", "Waiting for you"], refused: ["no", "You said no"], allowed: ["done", "Allowed"] };
...
function atCard(data, steps) {
  ...
  const p = s.kind === "ask" && PILL[s.state] ? `<span class="pill ${PILL[s.state][0]}"><i></i>${PILL[s.state][1]}</span>` : `<span class="pill idle">${WORD[s.kind]}</span>`;
  const cost = [s.cost?.display ?? "", known.length ? `${money(tot)} so far, ${dur(time)} in` : `${dur(time)} in`].filter(Boolean).join(" · ");
  const mid = messageOf(s);
  const jump = mid != null && document.querySelector(`#conversation [data-i15="${CSS.escape(String(mid))}"]`) ? `<button type="button" class="btn ghost sm" data-act="tljump17c" data-mid="${esc(mid)}">Show it in the conversation</button>` : "";
  const answer = v.helper?.waiting?.length ? '<button type="button" class="btn sm" data-act="hpopen17c">Answer in Activity</button>' : "";
  return `<div class="tlat17c" aria-live="polite"><div class="tla-h17c"><b>At step ${T.at + 1} of ${steps.length}</b>${p}</div><h3>${esc(v.title)}</h3><dl class="kv"><dt>Who</dt><dd>${esc(v.who)}</dd><dt>Details</dt><dd>${esc(v.detail)}</dd><dt>Took</dt><dd>${v.secs ? dur(v.secs) : "no time"}${s.at ? ` · at ${esc(clock(s.at))}` : ""}</dd><dt>Cost</dt><dd>${esc(cost)}</dd></dl>
    ${s.had ? `<p><b>What it had</b>${esc(s.had)}</p>` : ""}${s.happened ? `<p><b>What happened</b>${esc(s.happened)}</p>` : ""}
...
function verify(data, steps) {
  ...
  let msg = '<b>Check the record</b><small>Each step is chained to the one before it, so a cut or an edit would show.</small>';
  if (T.ver === 1) msg = "<b>Checking each link…</b><small>Every step carries the fingerprint of the one before.</small>";
  const intact = T.ver === 2 && c?.ok && data.chain?.entries > 0;
  if (intact) msg = `<b>Record intact</b><small>All ${data.chain.entries} steps link up, from ${esc(clock(first))} to the latest. Nothing was cut or rebuilt.${level() >= 2 ? `<br><code>chain head ${esc(String(c.tip).slice(0, 8))}…${esc(String(c.tip).slice(-4))} · sha-256</code>` : ""}</small>`;
  else if (T.ver === 2 && c) msg = `<b>${esc(c.reason)}</b>`;
  return `<div class="tlver17c ${intact ? "ok17c" : ""}">${ic(intact ? "check" : "shield", "s")}<span class="grow">${msg}</span>${T.ver === 1 ? "" : `<button type="button" class="btn sm" data-act="tlver17c">${T.ver === 2 ? "Check again" : "Check"}</button>`}</div>`;
}
...
export function timelineBody() {
  ...
  if (!steps.length) { stop(); return '<div data-art17="art17-timeline"></div><p class="empty">No task in this conversation yet. When a Trunk works on something, every model call, tool step and approval shows here, and you can replay it.</p>'; }
