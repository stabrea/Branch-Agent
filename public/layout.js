...
import { editing, sameCard } from "/same-card.js";
...
function moveCollab() {
  ...
  for (const [part, slot] of Object.entries(homes)) {
    ...
    // Q207: a part drawn the same as the one already home is dropped, so an unchanged page stays quiet. One the owner
    // is typing in stays too, until they leave the field (the refresh after that draws it again).
    if (sameCard(was, node) || editing(was)) { node.remove(); continue; }
    was?.remove();
    host.append(node);
  }
}
...
function buildRail() {
  ...
  /* The old Settings button becomes the gear beside search. The old Conversation button goes, as in the sample: a
     conversation opens from New conversation or its row in the side list, and the top bar's crumb is
     public/topbar-crumbs.js (DG-099). */
  ...
  document.querySelector('.nav[data-view="chat"]')?.remove();
