...
async function renderUpdates() {
  ...
  // Dogfood F9: the version rows are never left blank. This runs on every refresh, so it redraws the last status the
  // desktop gave rather than blanking it until the next one comes (NAS 20e1eb3); a browser has none, so it is null.
  showBuild(lastUpdateStatus);
