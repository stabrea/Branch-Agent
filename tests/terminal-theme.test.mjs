...
test("the style carries the depth, and a plain terminal gets no colour and no cursor movement", () => {
  ...
  const forced = resolveStyle({ TERM: "dumb", FORCE_TTY: "1" }, {}, "linux", "");
  assert.deepEqual([forced.cursor, forced.decorations, forced.color], [true, true, true],
    "an explicit full terminal view overrides an inherited TERM=dumb");
