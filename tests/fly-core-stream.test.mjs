 ...
 */
/** Worked out on first use, inside a test, so loading this file costs nothing. */
let measured;
const stream = () => (measured ??= summarise([1, 2, 3, 4, 5]).blocks);
const late = (name, from, to, field = "success") =>
  stream().slice(from, to).reduce((sum, block) => sum + block[name][field], 0) / (to - from);

test("L1 the core's picks improve over repeats", () => {
  const blocks = stream(), half = blocks.length / 2;
