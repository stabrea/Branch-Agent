...
/* NAS a870cea: the window's side, through the real public/app/shell/autoupdate.js (what the new window's main.js starts in
   the desktop app) and the real route (only the desktop bridge and the timers are stand-ins). After one release fails,
   update by itself still looks again when a look is due, and the next release is installed as soon as a look finds it. */
import { noteUpdateCheck } from "../dist/index.js";
import { autoUpdater } from "../public/app/shell/autoupdate.js";

/** Set by the test once the page is loaded: resolves when its update look (autoupdate.js `busy()`) is done. */
...
test("after one release fails, update by itself still looks when a look is due, and installs the next one (NAS a870cea)", async (t) => {
  ...
  const api = async (path, body) => {
    ...
    if (!response.ok) throw Object.assign(new Error(answer.error ?? String(response.status)), { status: response.status });
    return answer;
  };
  const toasts = [];
  const updater = autoUpdater({ desktop, api, toast: (words) => toasts.push(words), ready: () => "ready",
    timers: { setTimeout: time.setTimeout, clearTimeout: time.clear } });
  settled = async () => {
    for (let i = 0; i < 400; i++) {
