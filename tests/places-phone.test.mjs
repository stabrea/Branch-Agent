...
test("DG-140: the Inbox's Needs you tab carries the live count, and hides it at none", async (t) => {
  ...
  /* the badge is set and read in one step, so the Inbox's own redraw cannot land in between: the count's observer runs
     as a microtask, and waiting on a microtask (not a timer) leaves no room for a fetch to answer */
