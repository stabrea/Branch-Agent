...
const homes = [
  ...
  ["context-heartbeat", "settings:automations"],
  ["context-sop", "settings:automations"],
...
test("each switch is on the screen that already owns its subject, not on a screen of its own", async (t) => {
  ...
  for (const [id, home] of homes) {
    ...
    /* DG-198: on Settings › Automations & inbox these two show from Advanced. */
    if (home === "settings:automations") await page.evaluate(() => globalThis.branchSettingsLevel.set("technical"));
    await card.waitFor({ state: "visible", timeout: 10000 });
    assert.ok((await card.locator(":is(h2, h3)").first().innerText()).trim().length > 0, `${id} has a title on ${home}`);
...
test("every word on these cards can be said in French", async (t) => {
  ...
  const missing = await page.evaluate((ids) => {
    ...
    for (const id of ids) {
      ...
      for (const node of card.querySelectorAll("h2, h3, p:not(.field-note), label, option, button"))
