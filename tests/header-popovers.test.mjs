...
/**
 * The open menu, measured: how much of it a click can reach, and how it and everything in it compare with the
 * same menu shown in its own place (taken out of the page's top layer for a moment, then put back).
 */
const measure = (page, selector) => page.evaluate((sel) => {
  ...
  for (let x = box.left + 3; x < Math.min(box.right, innerWidth) - 3; x += 12)
    for (let y = box.top + 3; y < Math.min(box.bottom, innerHeight) - 3; y += 12) {
      if (corner(x, y)) continue;
      points++;
      const hit = document.elementFromPoint(x, y);
      if (!menu.contains(hit)) { covered++; under.add(hit?.id || String(hit?.className) || hit?.tagName); }
    }
  const look = () => [menu, ...menu.querySelectorAll("*")].filter((el) => el.getClientRects().length).map((el) => {
  ...
  const shown = look();
  const inline = menu.style.cssText, lifted = menu.hasAttribute("popover");
  if (lifted) { menu.removeAttribute("popover"); menu.style.cssText = ""; }
  const home = look();
  if (lifted) { menu.style.cssText = inline; menu.setAttribute("popover", "manual"); menu.showPopover(); }
  return { points, covered, under: [...under].slice(0, 4), shown, home };
}, selector);
