...
- Works with other programs and keeps running: an OpenAI-style endpoint (`/v1/chat/completions`, `/v1/models`) and live event streams; type while a task is still working and the message waits its turn; specialists can keep working in the background; one-file backup and restore; a health check that says what is wrong and what to do; webhooks and triggers for event-driven automation with external apps.
