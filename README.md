...
- Measuring it honestly, offline: scorers that decide a task without a model (exact, contains, pattern, JSON shape, a number within a tolerance, a file, whether a tool was used with particular arguments, a budget for rounds, time and money, and whether it actually finished) plus one that asks a model against a rubric; gates that fail a run and stop a release script; public test sets read from files you already have on this computer — SWE-bench, GAIA, APPS/MBPP/HumanEval, saved web-task pages and terminal-bench — with OSWorld, WindowsAgentArena and AndroidWorld named as not supported and why; written-down experiments that run several tasks at once, pick up where they stopped, keep the best of N tries, and report a difference with the range it is likely to be in; per-tool checks that need no model; and scripted model and tool doubles other people can write their own tests with.
...
- Ten more chat services from one list: Mattermost, Rocket.Chat, Google Chat, Microsoft Teams, Zulip, Feishu/Lark, DingTalk, WeCom, LINE and Viber all work through one connection described in `data/channels.json`, with Matrix, Signal (through signal-cli you install yourself), Facebook Messenger and Instagram beside them; each keeps the same pairing codes, allowlist, reply-when-mentioned, retries and quiet hours as Telegram, and one message can go out to several chats at once.
...
- Reading the documents people actually have: Word files, spreadsheets, slide decks, OpenDocument files, e-books, rich text and PDFs are all read on this computer with nothing installed, keeping their headings, sheets, slides and pages so a quotation can say where it came from. What cannot be read is said plainly — a PDF locked with a password is refused, and a PDF that is pictures of text says so rather than pretending. Ask a question of one file and get the answer with its heading and page, or hold two files up against each other and hear in plain language what changed. What a conversation taught can be written up as fact cards you accept into a knowledge base.
- Memory that knows what kind of thing it remembers: a preference, something about a person or the world, a hint about how to do something, a project note, or a scribble made while doing one job — and how long each is meant to last, so scribbles are cleared when the job ends unless you keep them. "Tidy my memory" runs every check at once and only ever suggests; `branch eval memory` measures whether the right fact is actually found.
...
- Being a proper MCP citizen, both ways round: other tools get a streaming connection that tells them when the toolbox changes, prompts with their blanks, read-only resources scoped by your approval settings, a "what would this do?" answer that does nothing, and a written record of the exact tool list they were shown; Branch as the one asking can sign in to a server that asks for it without ever seeing the key, shows a page such a server sends inside a frame that can do nothing, offers it as a card in the side pane, lets you try a server out from Settings before keeping it, and can be told to start a server only when a task actually calls one of its tools rather than every time Branch opens.
- Optional MCP tools and browser automation through explicit configuration.
- A browser that describes itself and mends itself: everything you can press on a page is numbered so the assistant says "press 3" instead of guessing, and the numbers survive the page redrawing itself; data comes off a page in the exact shape asked for or is refused by field name; an action is tried four different ways before it gives up, and which way worked is written down. With your say-so for one task, Branch can work in the browser you already have open so signed-in websites know you — banks and password sites always refused, your own tabs never touched, and it turns itself off after fifteen minutes. Search goes to the free fallback, your own SearXNG, or Brave, Tavily, Exa or Serper with the key in the locker.
...
- Specialists that work in different ways: one thinks a line out loud before every step (kept on the task, never in the answer), one plans its piece of work first, one reviews and cannot change anything however it was set up, one looks things up and has to name its sources, and one writes code with the tools already to hand.
- Changing several files at once, safely: see the whole change before a line is written, then all the files change or none of them do, each kept in its history so one can be put back on its own, binary files refused — and the check you set up for the project (your tests, your linter) runs straight afterwards and tells the assistant what it broke.
- Programs that keep going after the step that started them — a preview server, a watcher — held under the same limits as any other command, with what they printed kept to read, and stopped for good when the conversation is thrown away or the app closes. Small scripts can be run in a program of their own, once you switch that on.
- A tool may hand its work over and finish later: the task carries on, and the answer arrives in the conversation when it comes.
- Flows: a saved list of steps shown as boxes and arrows, with a way in over HTTP for other programs and a note sent out as each box finishes.
- Skills that suggest their own improvements and never take them: you see the lines that changed, try the new version against your last few tasks as a practice run where nothing is really done, and then keep it or throw it away. Skills can also live as files in a folder you keep in version control.
- Plugins from a folder or one file on this computer — the manifest and the fingerprint of the code shown before anything is copied in, and a file that changes afterwards noticed. No remote shop.
...
- **Unlimited tools, a small message.** Every tool on the computer is searchable, whether a dozen or a thousand: a handful travel with each round in full, a few dozen as one line each, and the rest are found by asking for them in plain words. What worked before is remembered, so an everyday request needs no looking at all.
...
- **A coder's toolbox.** A map of the project that puts the files most likely to hold the answer first; the language server and the debugger you already have, spoken to directly; a risky plan tried on a parallel copy and its difference shown before it comes back; points to come back to, with undo and redo; and any service that describes itself in OpenAPI turned into tools one operation at a time.
...
- **One switch that shuts everything down, and a few ways to spend less.** Lockdown makes every tool wait for your yes and turns off everything that reaches past the app — and turning it off puts back exactly the settings you had. The same question asked twice can be answered from what was kept, for nothing; a whole set of questions can be handed over at once where the service takes them, at about half the price; and every task now says which project it was done under, so you can see what each one costs. Other programs can read the app's own description of its web API at `/api/openapi.json`.
