...
- An app shell you can find your way around: every section is one click away in the icon column (never a drop-down), saved conversations sit in their own rail grouped by day, Ctrl+K finds any section, conversation or action, and Appearance lets you set the theme (Forest, Daylight, or follow this computer), the highlight colour, text size, spacing, lettering, stillness and whether the acorn shows.
