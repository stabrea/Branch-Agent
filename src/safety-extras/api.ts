...
import { beginCodeSetup, codeApprovalsView, confirmWithCode, finishCodeSetup, removeCodeSetup, saveCodeSetup } from "./code-approvals.js";
...
const SwitchSchema = z.object({ part: SafetyPartSchema, mode: z.enum(["off", "when-needed", "on"]) }).strict();
...
async function codesRoute(deps: SafetyHttpDeps, path: string): Promise<unknown> {
  const { store, owner } = deps.runtime;
  if (path === "/api/safety-extras/codes") return { codes: saveCodeSetup(store, owner, await deps.readBody()) };
  if (path === "/api/safety-extras/codes/begin") return beginCodeSetup(store, owner);
  if (path === "/api/safety-extras/codes/finish") {
    const { code } = CodeSchema.parse(await deps.readBody());
    if (!(await finishCodeSetup(store, owner, code))) throw new SafetyHttpError(400, "That code did not match. Check the time on your phone and try the next code.");
    return { codes: codeApprovalsView(store, owner) };
  }
  if (path === "/api/safety-extras/codes/remove") { removeCodeSetup(store, owner); return { codes: codeApprovalsView(store, owner) }; }
  if (path === "/api/safety-extras/codes/confirm") {
    ...
    if (!(await confirmWithCode(store, owner, input.sessionId, input.fingerprint, input.code)))
      throw new SafetyHttpError(400, "That code did not match, or it was already used. Wait for the next code.");
