...
export const notIntegratedBenchmarks: readonly { id: string; name: string; needs: string }[] = [
  ...
  { id: "browsergym-live", name: "BrowserGym live environments (MiniWoB, WebArena, WorkArena)", needs: "MiniWoB, WebArena and WorkArena are not datasets but servers that have to be running, whose pages change as the agent works and whose scoring reads the server's own state. Branch Agent downloads nothing and starts no server. Tasks that can be saved as ordinary pages are handled by the web-tasks adapter instead; the rest need those servers." },
];
