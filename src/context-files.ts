...
/** Finds one slot's file under whichever of its names is present, or nothing at all. */
export function findFile(folder: string, key: SlotKey): LoadedFile | null {
...
export interface AssembledContext { text: string; reports: SlotReport[]; bytes: number }
 ...
 */
export function assembleContext(folder: string, settings: ContextFileSettings): AssembledContext {
  ...
  for (const slot of slots) {
    ...
    const found = findFile(folder, slot.key);
  ...
  return { text: prompt(carried, announced), reports, bytes: used };
}
...
export function contextFileStatus(store: Store, owner: string, folder: string): SlotReport[] {
  return assembleContext(folder, contextFileSettings(store, owner)).reports;
}

/** Puts the owner's files in front of the model, and writes down what was carried. */
export function contextFileInstructions(store: Store, context: ToolContext): string {
  const settings = contextFileSettings(store, context.owner);
  if (!Object.values(settings.files).some((value) => value !== "off")) return "";
  const built = assembleContext(context.workspace, settings);
...
export function registerContextFiles(registry: ToolRegistry, store: Store): void {
  ...
  registry.register({
    ...
    execute: async ({ file }, context) => {
      ...
      const found = findFile(context.workspace, file);
