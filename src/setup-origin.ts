 ...
 *
 * The window sends `x-branch-origin: setup` on every request while setup is open (public/app/core/api.js). The mark
 * follows everything such a request starts, like the short-lived key's mark (src/key-context.ts). Anything that request
 * leaves running for longer (a timer, a listener) would carry the mark too, so the mark is one shared object that the
 * next request without the header closes: setup is over once the window asks for anything without it.
 */
...
/** Called once per request the key let in: the header opens (or keeps) setup's mark, its absence closes it. */
export function noteSetupOrigin(header: string | string[] | undefined): void {
  if (header !== "setup") { current.open = false; return; }
