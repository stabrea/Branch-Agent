...
export function collabState(app: Branch): unknown {
  const owner = app.runtime.owner, scope = app.store.profiles.scope();
  return {
    profile: { active: app.store.profiles.active(), all: app.store.profiles.list(), isOwner: app.store.profiles.isOwner() },
...
export async function collabApi(app: Branch, request: IncomingMessage, path: string, body: ReadBody):
  ...
  if (post && path === "/api/calendar") return app.calendar.configure(app.runtime.owner, await body());
