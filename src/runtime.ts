...
import { boundPictures, picturesKeptInView, takenPictureWords } from "./visual-window.js";
...
${run.output.slice(0, 6000)}`;
  ...
  private async showPicture(run: Run, messages: Message[], ids: (number | null)[], outcome: unknown, route: ModelRoute): Promise<void> {
    ...
    try {
      ...
      const taken = boundPictures(messages, picturesKeptInView, new Set(messages.filter((one) => one.content === takenPictureWords)));
