...
export async function updateLogItem(): Promise<ReportItem> {
  const text = await readFile(join(updateScratchDir(), "apply-update.log"), "utf8").then(
    (log) => redactForLog(log.split(/\r?\n/).slice(-updateLogLines).join("\n")),
...
async function failureReport(ctx: DiagnosticContext): Promise<{ name: string; path: string; base64: string }> {
  ...
  const items = (await gatherReport(reportSources(ctx, log))).filter((item) => updateItems.has(item.id));
