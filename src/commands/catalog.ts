...
export const COMMANDS: readonly CatalogCommand[] = [
  ...
  // a map reads the whole folder and a tour may ask a model, so it starts tasks rather than only looking.
  entry("learn", ["understand", "map"], "[code|documents] [folder or knowledge base]", "a map of something and a guided walk through it, in plain words", [...W, "terminal", "dashboard"], "run", { bareLooks: true, route: { method: "POST", path: "/api/learn/map" } }),
];
