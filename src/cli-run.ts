...
export function parseRunArgs(argv: string[]): RunFlags {
  const flags: RunFlags = { prompt: "", json: false, attach: [], plan: false, verify: false, dryRun: false };
  const words: string[] = [];
...
/** Applies `--preset`: the same saved approval setting the app's settings screen changes. */
export function applyPreset(store: Store, owner: string, name: string): string {
  const known = policyPresets().map((preset) => preset.id);
  if (!known.includes(name as PolicyPresetName)) throw new Error(`--preset takes one of: ${known.join(", ")}`);
