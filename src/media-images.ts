...
export function promptText(request: ImageRequest): string {
...
export async function generateOpenAi(
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ model, prompt: promptText(request), size: request.size, n: 1, response_format: "b64_json" }),
    signal,
    redirect: "error",
  });
...
export async function editOpenAi(
  ...
  form.append("prompt", promptText(request));
...
export async function generateGemini(
  ...
  const parts: Record<string, unknown>[] = [{ text: promptText(request) }];
