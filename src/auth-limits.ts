...
/** Which place a request came from, for counting wrong tries: its address, or "local" when unknown. */
export function requestSource(remoteAddress: string | undefined | null, forwarded?: string | string[] | undefined): string {
  const first = Array.isArray(forwarded) ? forwarded[0] : forwarded;
  const claimed = String(first ?? "").split(",")[0]?.trim();
  return (claimed || remoteAddress || "local").replace(/^::ffff:/, "").slice(0, 60);
}
