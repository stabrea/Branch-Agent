...
/**
 * Something that can replace references of its own inside a value. The locker asks each of these
 * before it looks at its own projects, so `secret://cmd/...` never reaches the project look-up.
 */
export interface ReferenceFiller {
  fill<T>(value: T, use: SecretUseNote): Promise<T>;
}
...
export const spawnSecretCommand: CliRunner = (executable, args, timeoutMs) =>
  new Promise((resolve) => {
    execFile(locateCommand(executable), args,
...
export function refusalFrom(entry: SecretCommand, outcome: CliOutcome): string | null {
  if (outcome.missing)
...
export class CommandSecrets implements SecretProvider, ReferenceFiller {
  ...
  async read(reference: string, use: SecretUseNote): Promise<string> {
    ...
    const refusal = refusalFrom(entry, outcome);
