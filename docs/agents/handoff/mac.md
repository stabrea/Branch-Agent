...
- `settleRun` now refuses to call an empty result a success. It is the only place the runtime
  ...
  the model loop; those are not tasks and are untouched.) A run that claims `completed` with
  nothing in its output becomes `failed` with a plain sentence (`src/empty-answer.ts`), worked out
...
`tests/models.test.mjs` asserted a thinking budget of 1,792 tokens, which was the old reply ceiling
of 2,048 less 256. `mac7/scoreboard` raised the ceiling to 8,192 (its F1 fix) without updating it, so
that one test was already failing on the base commit; it now follows `replyCeiling` instead of
restating the number.

`mac7/empty-completion` is **not** merged into `mac/cross-platform`, and `wave2/integration` was
deliberately not merged into it: the branch is pinned to the tree the measurement was taken on so
the evidence still lines up.
