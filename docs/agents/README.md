...
0. **The list.** [Issue #103](https://github.com/stabrea/Branch-Agent/issues/103) is the public, priority-ordered page: built / merged-not-released /
   being built now / next / decided against. **Keep it current in the same breath as the work** — when a
   branch starts, tick nothing but say so under "Being built right now"; when it merges, move its bucket
   up; when a release goes out, rewrite "(merged, ships in X)" to "(X)". It is regenerated by
   `scripts/make-list.py` in the scratchpad from the theme issues, so the issues stay the source of truth.
...
- **`npm run build` can leave `dist/` stale after a merge, silently.** On 2026-09-16 the merge of
  ...
  a 401 and the cause looked like a missing allowlist entry that was in fact present in the source.
  After merging, check that something you know changed is really in `dist` (`grep` for it) before
  trusting a test run, and delete `dist/` outright when anything looks wrong. A stale `dist` can
  produce a false green as easily as a false red.
