...
Installed skills are single-file SKILL.md documents stored per owner with immutable versions. Editing appends a version; activation, rollback, disable and removal check an expected revision and run in one transaction. A task's initial request lists only the enabled skills' names, descriptions and pinned versions; the full document loads only through an explicit `skills.read` for a listed id and version. Each run snapshots that catalog when it starts, so later edits do not change an active task, and removal blocks further reads. Skill metadata never grants tool permissions, and bundled scripts or assets are not installed.
