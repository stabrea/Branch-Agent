...
- **When every key already rests**, the connection answers as rate limited until its first key is ready, so the task moves to the next connection in the fallback order. A sign-in account at its limit is different on purpose: the task stops and says so (see below), and does not move to another connection unasked.

#### The terms decision (read at the sources on 2026-09-17)

- OpenAI's Terms of Use forbid using the services to "circumvent any rate limits or restrictions" (https://openai.com/policies/row-terms-of-use/; the page refuses automated reads, so the wording was taken from the search index's copy), and OpenAI sets API rate limits "at the organization level and at the project level, not user level" (https://developers.openai.com/api/docs/guides/rate-limits), which also says to follow `Retry-After`.
