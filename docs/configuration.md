...
**With nobody to ask.** A task nobody can answer — a script's `branch run`, a schedule, a trigger, a chat app, another AI tool over MCP or A2A — finishes with status `completed` and the plan as its answer, and changes nothing. The plan is saved, so saying "go ahead" in that conversation later carries it out. This is the same reading of "nobody can be asked" the tests question uses. `--plan` on `branch run` is not this switch: it works out a plan and then carries it out, and the sentence under `branch run --help` says so.
