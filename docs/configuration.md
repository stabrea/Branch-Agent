...
- **When every key already rests**, the connection answers as rate limited until its first key is ready: if that is within the retry limits the task waits that long and asks again, otherwise it moves to the next connection in the fallback order. A sign-in account at its limit is different on purpose: the task stops and says so (see below), and does not move to another connection unasked.
