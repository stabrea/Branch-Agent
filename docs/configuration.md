...
the box you type in pulls that passage into the task.
...
owner chose, returns something that answers like every built-in connection. It is handed the
network check to call rather than being trusted to make requests on its own terms, so the owner's
