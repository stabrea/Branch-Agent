...
week). Nothing is published anywhere: the link only works on this computer, because the app listens
on this machine's own address. `GET /api/shares` lists them and
`POST /api/shares/:id/revoke` stops one.
...
step. Resuming a workflow that is waiting on an approval is you saying yes. The same five things
are tools (`workflows.create`, `.list`, `.run`, `.pause`, `.resume`) under the
`workflows.manage` and `workflows.read` permissions. **Weekly review** ships as an example:
collect what finished, write the review, keep it in memory, and send it on.
...
three by default). A conversation only ever has one task working, so its others wait their turn.
...
removes one. Only the owner may add or remove people. While somebody's profile is switched on, the
conversation list, saved conversations and the Memory view are theirs and not the owner's, a task
they start is filed under their name, and the secrets locker and projects are refused in plain
words. **Be honest about what this is:** separation on one computer, not separate accounts. There
is no syncing, the assistant still works with the owner's models, tools and settings, anything the
assistant decides to remember by itself during a task is filed under the owner, and anyone who can
open the files on this machine can still read everything. The PIN keeps profiles apart; it does not
