...
`createBranch({ reliability })` accepts `modelStallMs` (5 s to 10 min, default 60 s: a model call that streams nothing for this long is treated as stalled), `localFirstReplyMs` (5 s to 30 min, default 300 s: for a connection on this computer — Ollama, LM Studio, llama.cpp or any other at `localhost`/`127.0.0.1`/`::1` — the wait for the *first* piece of each reply, since the model may be loading into memory; after anything is heard the ordinary `modelStallMs` applies, and hosted connections are unchanged. After 10 seconds of silence the task shows "Waiting for the model on this computer…" (event `model.loading`). The owner changes it with `localFirstReplySeconds` on the knobs card below. If a model on this computer has still said nothing when that wait runs out, it is tried again once, for a short grace only (a tenth of the wait, at most 30 seconds), so the whole wait is at most the setting plus the grace; then the next connection is used if `stallRecovery` allows falling back, and otherwise the task ends with a plain sentence saying the model on this computer didn't start answering and what to try), `stallRecovery` (`retry` twice then fall back, `fallback`, or `fail`), `toolTimeoutMs` (default 90 s: a single tool call is stopped after this), and `toolResultChars` (default 12,000: longer tool results are clipped for the model; the full result stays in the task's trace). Events: `model.stalled`, `model.stall_recovery`, `tool.stalled`, `tool.result_clipped`, `context.shrunk`.
...
When a step needs your yes, Branch asks the **editor**, with `session/request_permission` naming the step in plain words and offering `allow` and `reject`. Your answer goes straight to the approval rules and the turn carries on or stops, and the turn ends with `end_turn`, `refusal` or `cancelled`. Tasks from an editor carry `source: "acp"` and are capped the same way A2A tasks are. Attachments are refused here too. `session/prompt` (and the app-server's `turn/start`) works only in a conversation an editor or program opened here with `session/new` (`thread/start`); any other is refused the same way as over A2A, as a JSON-RPC error (`-32000` over ACP, `-32600` over the app-server, the codes each already uses for a refused request) whose message is that sentence. Such a conversation carries on after Branch restarts. An editor over ACP and a program over the app-server are both local connections you start yourself, and each may carry on a conversation the other opened.
...
- **Ask first** — reading is free; changing a file, running a command or acting on the web waits for
  your yes. It is the same as the *Ask before changes* setting, for this conversation only. A small
  script the assistant wants to run (`code.run`) is asked about every time, *Once* only: a yes for one
  script never lets the next through (a program on your list, `process.start`, asks too). Under
  Lockdown scripts are refused; in the other modes they follow the scripts permission as before.
...
| Messages | `trunk.message` lets a Trunk write to another from its own conversation only. Branch signs the message, it waits until the other is free, the answer comes back later, a failure that a second try can help is tried once more, and a chain stops three messages deep. One task sends at most three messages, and all Trunks together at most thirty an hour. When the task that reads a message stops to ask you something, the message is not failed: a card at the top of the window says "Ben is waiting for your answer", quotes whose message it is, and offers **Answer** (your next message in that Trunk's conversation carries it on, and its answer goes back to the sender) or **Not now** (the sender is told its message was not answered). Nothing you type there is sent back until you press Answer; a chat app's message or a short-lived key's never answers it. `POST /api/trunks/messages/<id>/answer` and `/decline`, the owner's alone. |
...
*Let it in* stays greyed until you tick **The code matches**, and the route itself holds to that:
`POST /api/devices/requests/<id>` with `{ "approve": true }` alone is refused (400) — a yes must say
`"codeMatches": true`, so a script or another screen cannot let a device in without the owner saying
they compared the codes. `{ "approve": false }` (Refuse) needs nothing more.
...
| | `maxModelRounds` | `null` (launch `maxModelRounds`, 12) | How many times one task may go back to the model before it stops, 2 to 60. When it runs out the task asks the model once more, with no tools, for the best answer it can give from the work it did, and ends with that answer followed by one plain sentence: how many rounds it took, what it spent them on (the same tool over and over, all its tool calls failing, and so on) and where this setting is. It never ends on the bare words "Maximum 12 model rounds reached" as it used to. The task is still recorded as having stopped at its limit rather than finished. A task working to a plan gets four more rounds a step on top of this, up to 40. Event: `rounds.exhausted` |
...
| Doing more in one go (`fewer-rounds`) | A coding task goes back to the model fewer times. Measured on this branch: 88-94% of a coding task's wall clock is waiting for the model, and packing the same tool calls into fewer rounds cut one task from 1,958 ms to 928 ms. Three things, switched together: the tools a coding task always needs (`files.read`, `files.grep`, `files.list`, `files.glob`, `files.edit`, `files.write`) are described in full from the first round, so it never spends a whole exchange searching for `files.edit` before it can start (measured: 14 of 30 of those places needed a search first, 1 of 30 with this on); the assistant is told in one line that it may ask for several independent things at once; and `files.read_many` reads up to 10 files in one call, each through the very same checks and the same 32 KiB ceiling as `files.read`, naming any path it could not read and still returning the rest. Like *Reading a file before changing it*, the behaviour is the same in *when needed* and *on* - there is no useful middle for how a loop behaves; what the two modes choose is whether `files.read_many` is described in full from the first round (*on*) or is a line in the index until the work calls for it (*when needed*). Ships off | `src/coding/fewer-rounds.ts`, `src/coding/read-many.ts` |
