...
It also switches off automations that start by themselves (every part under Automations) and
routines a Trunk owns. **Lockdown wins over every saved switch** (mac7/lockdown-fix): each of these
...
screen and keyboard and borrowing your browser (`shell.execute`, `code.execute`, `remote.execute`,
`process.manage`, `desktop.*`, `browser.borrow`) are refused outright in `Runtime.checkPolicy`, whatever
a rule says, and a rule saved while Lockdown is on cannot let any other tool past without a yes.
...
other keys, with it off a connection with no pick refuses the Trunk rather than using your default. A Trunk saved as a file (`branch-trunk/1`) carries who it is and never its conversations,
