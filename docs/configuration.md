...
| Memory, `CLAUDE.md` / `AGENTS.md` / `SOUL.md` | Context files (`AGENTS`, `CLAUDE`, `GEMINI`, `SOUL`, `USER`, `IDENTITY`, `MEMORY`, `HEARTBEAT`, `TOOLS`, `SOP`) are handed to the context-file loader when the launch provides one (`contextFiles` in the move-in options). Until then, and for other notes, they become saved facts (long text is split, at most 3800 characters each); instructions become preferences. |
