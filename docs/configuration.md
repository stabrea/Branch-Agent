...
| `POST /api/local-models/one-button` | The button itself: `{ "size": "small" \| "medium" \| "large", "agreedPlan": "<the plan's line>" }`. |
...
- **How each system installs it.** A Mac with Homebrew uses `brew install ollama` or `brew install --cask lm-studio`; Windows with winget uses `winget install --id Ollama.Ollama` or `--id ElementLabs.LMStudio`. Homebrew and winget check their own package before installing it. Without one of those, Ollama comes from Ollama's own release: Branch reads the SHA-256 the publisher publishes beside the file, downloads the file from that same release, and throws it away if the two do not match. On a Mac it then asks macOS itself (`codesign --verify --strict`, `spctl --assess`) whether the program is signed by its publisher and notarised by Apple before anything is put in `/Applications`. On Linux the publisher's install script is saved to a file, checked against the same published SHA-256, and run as a file — never piped into a shell. On Windows the checked installer is opened so you can click through it, and Branch waits while you do (up to twenty minutes) before looking for the program again. Every command is an argument list; there is no shell line anywhere in this path.
- **What Branch will not do.** LM Studio publishes no checksum for a direct download, so without Homebrew or winget Branch says so and points at lmstudio.ai rather than fetching it. Downloads only ever come from `github.com`, `objects.githubusercontent.com`, `release-assets.githubusercontent.com` and `ollama.com`, each redirect checked again by hand. When a step fails, Branch says which step and what the program said, and stops: it never reports success it has not seen. After an install it looks the program up again the ordinary way and refuses to carry on if it is not really there.
