...
Every program is started with a list of arguments, never a shell line. yt-dlp always gets `--ignore-config` (no settings file can add a command to run afterwards) and `--` before the address (an address can never be read as an option); only `http` and `https` addresses without a name or password are accepted, and the address is checked against the network policy before yt-dlp starts. yt-dlp then makes its own connections, which the network policy cannot follow hop by hop. Tests use a stand-in for both programs; the ffmpeg lists were also run once by hand with ffmpeg 8.1 on the owner's Mac against a generated four-second clip (four pictures, the sound, WAV and MP3 all came out), and every yt-dlp option was checked against yt-dlp 2026.07.04's own help. Nothing was downloaded.
