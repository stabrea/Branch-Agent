...
## Loops, folder instructions and trusted folders

**Going round in circles.** Every task watches its own tool calls. The same tool with exactly the same details a third time gets a warning beside its result (`loopWarning`); a fifth time it is not run and the model is told why. A call that keeps giving back exactly the same result is refused sooner (from the fourth time), and two or three calls going back and forth (read, write, read, write) are warned about after two rounds and refused after three. Tools meant to be asked again while something finishes — a running program's output (`process.read`), a status, a list — get three times the room, judged from the tool's name only. After three refusals the task ends with one sentence starting "Stopped:", and every call the model asked for still has an answer in the conversation, so the next message carries on normally. Events: `loop.warned`, `loop.blocked`, `loop.stopped`. The limits are fixed in `src/loop-guard.ts`; there is no setting for them.

**The folder's own instructions.** Branch reads `AGENTS.md` from the workspace down to the folder a task works in (the active project's `folder`), one file per folder; `CLAUDE.md` and then `GEMINI.md` are read only where there is no `AGENTS.md`. A deeper folder's file is read the first time the task touches a file there and arrives beside that tool's result (`folderInstructions`), once per task. A line holding only `@path/to/other.md` pulls that file in, up to five levels deep; a file that would include itself again, a path outside the workspace, and `@` lines inside code blocks are left out with a note. Each file keeps at most 16,000 characters and one task at most 48,000; a file over the file tools' 32 KiB is not read. The same checks as the file tools apply (no links, no secret-looking names, `.branchignore`), and `.gitignore` is respected when there is no `.branchignore`. Events: `instructions.loaded`, `instructions.failed`.

**Trusted folders.** A folder's instructions are read only once the owner trusts it. `GET /api/folder-trust` lists the workspace and every project folder with what each carries (notes, AI-tool servers from `.mcp.json`, skills under `.agents/skills`, `.claude/skills` or `.gemini/skills`, hooks from `.claude/settings.json` or `.gemini/settings.json`) and whether it is `trusted`, `untrusted` or `unknown`; `POST /api/folder-trust { folder, decision: "trust" | "distrust" }` records an answer for a folder written relative to the workspace (`""` is the workspace itself). A decision covers everything inside the folder and the closest one wins. Not decided yet: nothing of the folder's own is read and approvals are unchanged; the chat screen asks, and a task writes `folder.trust_needed`. Not trusted: nothing is read from it. When the folder a task works in (the workspace, or the active project's `folder`) is not trusted, every change in that task waits for a yes, even with approvals switched off; deny and ask rules are kept and standing yeses are dropped. Reading a folder's notes is decided folder by folder, but the approval fallback follows only the task's own folder: distrusting `vendor/` inside a trusted workspace keeps its notes out without making writes there ask. Branch does not load AI-tool servers, skills or hooks from a folder at all today, so for those the list is for the owner's information. A short-lived key cannot change these answers. Every answer is written to the record as `policy.changed`.

**macOS and Linux.** All three work the same on every system. Folder decisions compare paths with letter case on macOS and Linux and without it on Windows; a note is read by the name `AGENTS.md`, which on the usual macOS disk (and on Windows) also finds `agents.md` while Linux needs the exact name, so the trust screen lists notes whatever their case. A note or a skills folder that is a link is never followed.
