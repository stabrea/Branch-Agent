...
**Inbound triggers** let external apps start work by posting JSON to a webhook URL. Create a trigger with a name, a prompt template, and an optional session ID; the assistant generates a URL `POST /api/triggers/:id/fire` plus a 24-byte secret. Requests are verified with either a bearer token (`Authorization: Bearer <secret>`) or an HMAC-SHA256 signature (`X-Branch-Signature: sha256=<hex>`). The prompt template supports `{{payload}}` (the entire request body as JSON) and `{{field.path}}` (nested values from the payload, e.g. `{{user.name}}`); placeholders are substituted before running. Each trigger has a rate limit (default 30 per minute) and a 256 KiB payload size limit. Attempts are logged with payload summaries and run IDs. The fire route does not require the session token; it carries its own secret.

**Outbound webhooks** notify external endpoints when Branch events occur (`run.completed`, `run.failed`, `schedule.fired`, `delivery.failed`, `trigger.fired`, `approval.needed`). Register a URL and optional secret; deliveries are JSON POST requests with an `X-Branch-Signature` header if a secret is set. Failed deliveries retry with exponential backoff (5 s, 10 s, 30 s) up to three times; after five consecutive failures, the webhook is auto-disabled with a status message. Deliveries can be tested without waiting for real events. All delivery attempts are logged with status and retry timing.

Authenticated routes: `GET /api/triggers` (list), `POST /api/triggers` (create), `GET /api/triggers/:id` (get), `GET /api/triggers/:id/log` (recent fires), `POST /api/triggers/:id/rotate-secret` (regenerate bearer token), `DELETE /api/triggers/:id/remove` (delete). Similarly for `/api/webhooks` with a `POST /api/webhooks/:id/test` (send a test payload) and `POST /api/webhooks/:id/enable` (re-enable after auto-disable). Trigger secrets are never returned; only the fire route needs them. Webhook secrets are not returned but are used for signature generation and stored encrypted if not provided on creation.
