...
Every setting, route and switch Branch Agent has, arranged in the same ten chapters as the
[owner's handbook](handbook/00-start-here.md). The handbook says what to do; this says exactly what
each thing is. A note of when something was built belongs in [CHECKPOINT.md](CHECKPOINT.md), not here.

## Contents

0. [Start here](#chapter-0--start-here) — handbook: [00-start-here.md](handbook/00-start-here.md)
1. [Connect a model](#chapter-1--connect-a-model) — handbook: [01-connect-a-model.md](handbook/01-connect-a-model.md)
2. [Everyday tasks](#chapter-2--everyday-tasks) — handbook: [02-everyday-tasks.md](handbook/02-everyday-tasks.md)
3. [Your memory and knowledge](#chapter-3--your-memory-and-knowledge) — handbook: [03-your-memory-and-knowledge.md](handbook/03-your-memory-and-knowledge.md)
4. [Permissions and safety](#chapter-4--permissions-and-safety) — handbook: [04-permissions-and-safety.md](handbook/04-permissions-and-safety.md)
5. [Reach it anywhere](#chapter-5--reach-it-anywhere) — handbook: [05-reach-it-anywhere.md](handbook/05-reach-it-anywhere.md)
6. [Automate](#chapter-6--automate) — handbook: [06-automate.md](handbook/06-automate.md)
7. [For builders](#chapter-7--for-builders) — handbook: [07-for-builders.md](handbook/07-for-builders.md)
8. [Troubleshooting](#chapter-8--troubleshooting) — handbook: [08-troubleshooting.md](handbook/08-troubleshooting.md)
9. [Glossary](#chapter-9--glossary) — handbook: [09-glossary.md](handbook/09-glossary.md)
10. [What Branch is not](#chapter-10--what-branch-is-not) — handbook: [10-what-branch-is-not.md](handbook/10-what-branch-is-not.md)
...
Those are the fields of `ModelSettingsSchema` (`src/models.ts`), saved under the setting `models`:
`activePreset` (which connection answers by default in this workspace), `fallbackOrder` (the
connections to try, in order, when one fails) and `cooldownMs` (how long a failed connection rests
before it is tried again). `reasoning` holds the default thinking effort.
...
`GET /api/providers/catalog` returns one preset for **every** service in `data/providers.json` — 38 of them today, listed in the table below — because it is built from that catalog rather than from a hand-written list. Each preset includes the service's address, well-known model identifiers and plain-language help text on where to obtain a key or how to set up a local service. The desktop and web interface use this catalog to guide model configuration, so adding a line to the data file adds a service to the screen.
...
### Asking the same thing twice: kept answers
...
**Settings → Voice** configures speech input and output. Writing out what you say can be done by an
OpenAI-shaped service, by Gemini, or by a speech program already installed on this computer; reading
replies aloud can be done by an OpenAI-shaped service, by Gemini, or by the voices that come with
Windows, which need no key, no account and no internet. The browser's own voice is still used where
the page can do it. The ChatGPT plan sign-in does not carry a key for the sound routes, so an
OpenAI-shaped route needs a key configured under **Settings → Model connection**.

API routes: `POST /api/voice/transcribe` (sound in), `POST /api/voice/speak` (words in, sound out),
`GET|POST /api/voice/settings`, `GET /api/voice/plan`, `GET /api/voice/voices`, `POST /api/voice/live`.
...
#### Talking to it, and where the sound goes
...
**Settings → Pictures and sound** holds three things, which are the three fields of
`MediaSettingsSchema` (`src/media-settings.ts`): `imageModel` (which model makes pictures — leave it
empty to use whatever the connected service usually uses), `folder` (where in your workspace finished
files are saved, `media` by default), and `imagePrices` (your own corrections to the per-picture
prices, used wherever a cost is shown). API: `GET|POST /api/media/settings`.
...
## Chapter 3 — Your memory and knowledge
...
### Traces, the counters page and the permission rules
...
### What is allowed, trajectories, the live feed, the month view and metering
...
stops one that is working, and `POST /api/queue/settings` sets `atOnce` — how many tasks may work at
the same time before the rest wait (one to eight, three by default), the one field of
`QueueSettingsSchema` in `src/run-queue.ts`. A conversation only ever has one task working, so its others wait their turn. A
...
`BriefSettingsSchema` (`src/brief.ts`) keeps two more fields Branch writes for itself and you never
set: `lastSentAt` (when the brief last went out) and `nextAt` (when it is next due). They are what
stops a brief going out twice after a restart, and they are in the saved record so you can see them.
...
### Specialists that work in different ways
...
### Work handed over to finish later
...
### Flows: workflows as boxes and arrows
...
### Skills that improve themselves, safely
...
### One flow inside another, and flow checkpointing
...
### What a project brings to a task
...
### The client library, issue context, and what the assistant was allowed to do
...
`BackgroundSettingsSchema` (`src/processes.ts`) is the switch and its limits: `enabled`, `allowed`
(the programs that may be left running at all), `maxRunning` (how many at once), `maxMinutes` (how
...
is allowed at all, where your Python is if you have one (`python`, a full path), whether a script
...
### The long tail: the API description, kept answers, whole sets, Lockdown, branches and projects
...
### The app's own web API, described
...
### A whole set of questions at once
...
The words the interface, this reference and the handbook all use for the same thing are listed in
[the handbook's glossary](handbook/09-glossary.md), with the jargon each one replaces. The table of
names the screens are held to — and the words that must never reach the owner — is in
[design.md](design.md), under *The glossary*; `tests/shell-ui.test.mjs` fails if any of the
machine-checkable ones appears in the rendered text of a section.

Nothing in this chapter is a setting. It is here so the reference has the same ten chapters as the
handbook, and so a word you meet in one has a definition in the other.

## Chapter 10 — What Branch is not

### Deliberately not built

- **embedded code editor** — the Documents and "Look inside" screens show code read-only with
...
- **configurable intent pipeline** — skill discovery and dispatch already choose what to do. A
  pipeline the owner configures would be a second, competing way to decide the same thing.
- **personal knowledge base on a graph database** — needs a graph store running alongside; the
  knowledge bases here do the same job on the SQLite file that is already there.
- **Gradio interface** and **side-panel chat** — Gradio is a Python web toolkit; the web
  app here is the interface. A browser side panel is an extension, not part of a local app.
- **C FFI** — calling native libraries from the assistant would put unsandboxed native code
  inside the app. Anything needing that is a program the owner runs through the shell tools.
- **ADB operator** — driving an Android phone over USB is not a local Windows desktop
  assistant's job.
- **Ultracode plugin lifecycle** — another project's plugin format. Branch has its own
  (`src/plugins.ts`, `src/plugin-catalog.ts`) with fingerprints and an explicit switch.
- **LiteLLM** — a Python proxy in front of many providers. The provider catalog and the
